...
/**
 * This interface allows for using a custom "Builder" base interface in your project. Please note
 * that a default
 * BuilderDelegate implementation is already provided. You do not need to provide a custom delegate
 * if you use
...
public interface BuilderDelegate<B> {
    ...
    /**
     * Implementation has to call the right method on the target builder in order to create an
     * instance of
    ...
    /**
     * @param value when a builder sets values on the target object, it will ask the builder
     * delegate to determine if
     * the value is a 'builder'. If it is, then it will be asked to be built and the result will be
    ...
    /**
     * Implementation has to determine if the provided Method is the actual 'build' method. That is,
     * the method that
